...
N_TRIALS = 5
OUTPUT_DIR = "roberta_autotune_outputs"
...
def objective(trial):
    ...
    args = TrainingArguments(
        ...
        save_strategy="epoch",        # *** แก้จาก no เป็น epoch ***
        learning_rate=lr,
        per_device_train_batch_size=bs,
        per_device_eval_batch_size=16,
        num_train_epochs=epochs,
        weight_decay=wd,
        load_best_model_at_end=True,  # *** แก้จาก False เป็น True ***
        metric_for_best_model="f1_macro", # *** ต้องระบุตัวนี้ด้วย ***
    ...
    trial_trainer = Trainer(
        ...
        # EarlyStopping จะทำงานได้แล้วเพราะมี metric_for_best_model และ load_best_model_at_end ครบ
...
# --- เก็บ optuna study ผ่าน callback (ชัวร์ที่สุด) ---
study_holder = {"study": None}

def save_study_callback(study, trial):
    # callback นี้ถูกเรียกทุกครั้งที่ trial จบ
...
final_args = TrainingArguments(
    output_dir=os.path.join(OUTPUT_DIR, "roberta_best_model_run"),
    eval_strategy="epoch",
    save_strategy="epoch",
    save_total_limit=1,
...
# ----------------------------
SAVE_DIR = os.path.join(OUTPUT_DIR, "roberta_best_model_saved")
